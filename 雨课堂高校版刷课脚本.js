...
// @version                   1.1
// @description               雨课堂高校版：支持视频学习、字体解密、题库与 AI 答题
// @author                    XiaobaiBye
// @license                   GPL3
// @match                     *://*.yuketang.cn/*
// @match                     *://*.gdufemooc.cn/*
...
(() => {
  ...
  const Utils = {
    ...
    parseProgressPercentages(text) {
      return [...String(text || '').matchAll(/(\d{1,3}(?:\.\d+)?)\s*%/g)]
    ...
    isProgressDone(text) {
      ...
      const percentages = Utils.parseProgressPercentages(String(text));
            ...
            <div class="panel" id="panel">
              ...
              <div class="body">
                <ul class="info" id="info">
                  <li style="color:#722ed1;font-weight:700;">✅ 正式版 v${Config.version}</li>
                  <li>🔓 <strong>字体解密：</strong>检测到雨课堂加密字体时先在浏览器内还原，失败再回退 OCR</li>
                  <li>⭐ 脚本支持：雨课堂所有版本</li>
  ...
  const AiWorkspace = {
    ...
    getActiveLeafCompletion() {
      ...
      const percentages = Utils.parseProgressPercentages(text);
      const markerSignals = [];
      const iconRefs = [];
...
`;
  ...
  class V2Runner {
    ...
    checkCompletionStatus(statusBox, statusText) {
      ...
      const percentages = Utils.parseProgressPercentages(String(statusText || ''));
      if (percentages.length) return Math.max(...percentages) === 100;
  ...
  async function boot() {
    ...
    try {
      panel = createPanel();
      panel.log(`雨课堂高校版刷课脚本v${Config.version} 已加载`);
